...
void APP_Init(void)
{
	...
	tid = rt_thread_create("ap01", app_ap01_thread_entry, RT_NULL,  
...
void app_uart_thread_entry(void *parameter) 
{
	...
	while(1)
	{
      ...
      else if(strstr(pBuff, "unsub")!=NULL)//取消订阅
      {
        u8 conn_id=0, sub_id=0;
        drv_mqtt_set_topic_state(conn_id, sub_id, TopicStateUnSub);//取消该话题的订阅
      }
      else if((pData=strstr(pBuff, "pwm:"))!=NULL)//PWM设置
      {
        pData+=strlen("pwm:");
        u8 pwm_val=atoi(pData);
        printf("debug set pwm=%d\n", pwm_val);
        app_motor_set_speed(pwm_val);
      }
