...
QJsonObject DrvCommon::readConfg(QString path, u8 *key_buff)
{
    ...
    file.close(); 
    QByteArray json_ba=read_ba;//
    if(key_buff)
    {
        json_ba=QByteArray::fromBase64(read_ba);
        u8 out_buff[1000]={0};
        int out_len=aes_decrypt_buff((u8*)json_ba.data(), json_ba.size(), out_buff, sizeof (out_buff), key_buff);
        if(out_len>= json_ba.size())
        {
            while(out_len>0 && out_buff[out_len-1]==0)//取出填充的0
            {
                out_len--;
            }
            json_ba.setRawData((char*)out_buff, out_len);
...
int DrvCommon::writeConfig(QString path, QJsonObject root_obj, u8 *key_buff)
{
    QJsonDocument json_doc;
    QFile file(path);  
    ...
    {
        u8 out_buff[1000]={0};
        int out_len=aes_encrypt_buff((u8*)json_ba.data(), json_ba.size(), out_buff, sizeof (out_buff), key_buff);
        if(out_len>= json_ba.size())
        {
            json_ba.setRawData((char*)out_buff, out_len);
            json_ba=json_ba.toBase64();
        }
    }
